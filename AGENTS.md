...
All development is done in the **devel tree** (`~/Documents/ksf_FA_CampaignBuilder`). Do **not** edit files in the UAT bind point directly.
...
7. **Deploy** to UAT by pulling in the Infrastructure bind point:
   ...
   ```
   cd ~/ksf_Infrastructure/fa_modules/ksf_FA_CampaignBuilder
   git stash -u
...
| Path | Purpose |
|------|---------|
| `~/Documents/ksf_FA_CampaignBuilder` | Devel tree — all development, testing, commits |
| `~/ksf_Infrastructure/fa_modules/ksf_FA_CampaignBuilder` | UAT bind point — deployment target, integration testing (if mirrored) |
